...
export const feesApply = async (req, res) => {
  ...
  try {
    ...
    if (monthsToApply.length > 0) {
      ...
      await connection.query(
        `DELETE FROM student_months WHERE admissionNumber = ? AND month IN (${delPlaceholders})`,
...
export const getPendingFees = async (req, res) => {
  const admissionNo = req.method === "GET" ? req.query.admissionNo : req.body.admissionNo;
...
/**
 * Get All Fee Records (Optionally filtered by admissionNo)
 */
export const getAllFeesRecords = async (req, res) => {
  const admissionNo = req.query.admissionNo || null;
  let connection;
